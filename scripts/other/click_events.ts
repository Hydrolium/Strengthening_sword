...
export function onClickUpgradeButton() {
    ...
    if(result == TestResult.SUCCESS) { // 강화 가능 상태일 때
        ...
        if(Math.random() < sword.prob) { // 성공하면
            ...
            if(Game.swordManager.current_sword_index == Game.swordManager.max_upgradable_index) Game.mainScreen.popupGameEndMessage();

        } else {
            
            const dropped_pieces = sword.pieces.map(
                piece => piece.drop()
            ).filter(piece => piece.count > 0); // 떨어진 조각 맵핑
            ...
            if(Math.random() < percent) { // [ 무효화 구체 ] 성공시
                ...
                Game.mainScreen.popupFallMessage(
                    Game.swordManager.calculateLoss(Game.swordManager.current_sword_index),
                    dropped_pieces,
