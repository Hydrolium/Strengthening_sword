...
import { Item, MoneyItem, PieceItem, Recipe, StatTestResult, SwordItem, SwordTestResult } from "./entity.js";
...
(function onClickFooterIcon() {
    ...
    $("#making-button").addEventListener("click", () => Game.makingScreen.show(Game.makingManager.makingContext));
    $("#stat-button").addEventListener("click", () => Game.statScreen.show(Game.statManager.statContext));
})();
...
export function onClickUpgradeButton() {
    ...
    switch(attempt.result) {
        case SwordTestResult.REJECTED_BY_MAX_UPGRADE: Game.mainScreen.popupMaxUpgradeMessage(); break;
        case SwordTestResult.REJECTED_BY_MONEY_LACK: Game.mainScreen.popupMoneyLackMessage(); break;
        ...
        case SwordTestResult.SUCCESS:
            ...
            break;
        case SwordTestResult.FAIL_BUT_INVALIDATED: Game.mainScreen.popupInvalidationMessage(attempt.dropped_pieces!); break;
        case SwordTestResult.FAIL:
            Game.mainScreen.popupFallMessage(
                Game.swordManager.calculateLoss(attempt.result_index),
                attempt.dropped_pieces!,
                Game.inventoryManager.getRepairPaper(),
                attempt.sword!.requiredRepairs
            );  break;
    }
}
...
export function onStatUp(statIdStr: string) {
    ...
    const statID = StatID[statIdStr.toUpperCase() as keyof typeof StatID];
...
export function onClickMakingButton(recipe: Recipe) {
    ...
    if(Game.inventoryManager.hasItems(recipe.materials)) {
        Game.makingScreen.animateLoading(
                (recipe.result instanceof SwordItem) ? 1200 : 700,
                () => {
                    Game.makingManager.makeWithRecipe(recipe);
                    if((recipe.result instanceof SwordItem)) Game.makingScreen.popupCreatedSwordMessage();
                }
            );
    }
}

export function onClickSwordItemSellButton(id: string) {
...
export function onClickSwordBreakButton(id: string) {
...
export function onClickSwordSwapButton(id: string) {
...
export function onClickWherePieceDroppedButton(pieceItem: PieceItem
) {
    if(isClicking) return;
    isClicking = true;
    setTimeout(() => {
    ...
    Game.inventoryScreen.popupWherePieceDroppedMessage(
        pieceItem, Game.swordManager.getSwordsByPieceId(pieceItem.id), Game.swordManager.getFoundSwordIndexes()
    );
}

export function onClickSwordInfoButton(id: string) {
    if(isClicking) return;
    isClicking = true;
    setTimeout(() => {
        isClicking = false;
    ...
    Game.informationScreen.popupSwordInfoMessage(Game.swordManager.getSwordWithId(id));
