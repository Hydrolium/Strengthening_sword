...
export class MainScreen extends Screen {
    ...
    render(context) {
        if ((context === null || context === void 0 ? void 0 : context.type) != ContextType.SWORD)
            return;
        const element_sword_image = $("#sword-image");
        const element_sword_number = $("#sword-number");
        const element_sword_name = $("#sword-name");
        const element_sword_prob = $("#sword-prob");
        const element_sword_cost = $("#sword-cost");
        const element_sword_price = $("#sword-price");
        const element_sell_button = $("#sell-button");
        const element_save_button = $("#save-button");
        element_sword_image.src = Game.Path[context.sword.id];
        write(element_sword_number, context.index);
        if (context.isMax)
            element_sword_number === null || element_sword_number === void 0 ? void 0 : element_sword_number.classList.add("hightlight");
        write(element_sword_name, context.sword.name);
        element_sword_prob === null || element_sword_prob === void 0 ? void 0 : element_sword_prob.setAttribute("enabled", `${!context.isMax}`);
        element_sword_cost === null || element_sword_cost === void 0 ? void 0 : element_sword_cost.setAttribute("enabled", `${!context.isMax}`);
        if (!context.isMax) {
        ...
        if (context.sword.canSave)
            visible(element_save_button);
        else
            invisible(element_save_button);
    ...
    popupFallMessage(loss, pieces, having_repair_paper, required_repair_paper) {
        ...
        if (having_repair_paper >= required_repair_paper) {
            ...
            popup.setFooter(`복구권 ${required_repair_paper}개로 복구할 수 있습니다. (${having_repair_paper}/${required_repair_paper})`, Color.SKY);
        }
        else {
            popup.setFooter(`복구권이 부족하여 복구할 수 없습니다. (${having_repair_paper}/${required_repair_paper})`, Color.RED);
        }
