...
export class InventoryScreen extends Screen {
    ...
    makePieceHoverMenuDiv(piece) {
        const created_div = createElementWith("div", { classes: ["hover_menu", "hover_menu_1"] });
    ...
    makeInventoryArticle(item) {
        ...
        if (item instanceof PieceItem)
            created_div.appendChild(this.makePieceHoverMenuDiv(item));
    ...
    makeRepairGroupSection(repairPaperCount) {
        ...
        created_repairGroup.appendChild(this.makeInventoryArticle(new RepairPaperItem(repairPaperCount)));
        return created_repairGroup;
    }
    makePieceGroupSection(pieceItemList) {
        ...
        pieceItemList.forEach(pieceItem => created_pieceGroup.appendChild(this.makeInventoryArticle(pieceItem)));
        return created_pieceGroup;
    }
    makeSwordGroupSection(swordItemList) {
        ...
        swordItemList.forEach(swordItem => {
            created_swordGroup.appendChild(this.makeInventoryArticle(swordItem));
        });
    ...
    render(context) {
        ...
        if (context.repairPapers > 0)
            inner.push(this.makeRepairGroupSection(context.repairPapers));
        ...
        if (context.swords.length != 0)
            inner.push(this.makeSwordGroupSection(Array.from(context.swords.items.values())));
    ...
    popupSwordItemSellMessage(sword, sellFunc) {
        ...
        popup.addColoredParagraph(new ColoredTextElement().add("획득 가능한 금액: ", Color.DARK_GRAY).add(sword.price, Color.GOLD).add("원", Color.DARK_GRAY));
        popup.addButton("판매하기", Color.BROWN, ButtonType.SELL, HoverEffect.INCREASE, sellFunc);
        popup.addCloseButton();
        popup.build();
        popup.show();
    }
    popupWherePieceDroppedMessage(pieceItem, swords, founds) {
        const popup = new Popup();
