...
export class InformationScreen extends Screen {
    ...
    popupSwordInfoMessage(id) {
        ...
        if (sword.price != 0)
            popup.addParagraphElement(new ColoredText("p").add("판매 가격: ", Color.DARK_GRAY).add(sword.price, Color.GOLD).add("원", Color.DARK_GRAY).build());
        else
            popup.addParagraphElement(new ColoredText("p").add("판매 가격: ", Color.DARK_GRAY).add("판매 불가", Color.RED).build());
        if (sword.requiredRepairs != 0)
            popup.addParagraphElement(new ColoredText("p").add("파괴 시 필요 복구권: ", Color.DARK_GRAY).add(sword.requiredRepairs, Color.GOLD).add("개", Color.DARK_GRAY).build());
