...
import { StatTestResult, SwordItem, SwordTestResult } from "./entity.js";
...
export function onClickMakingButton(recipe) {
    if (isClicking)
        return;
    isClicking = true;
    setTimeout(() => {
    ...
    if (Game.inventoryManager.hasItems(recipe.materials)) {
        Game.makingScreen.animateLoading((recipe.result instanceof SwordItem) ? 1200 : 700, () => {
...
export function onClickWherePieceDroppedButton(pieceItem) {
    if (isClicking)
        return;
    isClicking = true;
    setTimeout(() => {
    ...
    Game.inventoryScreen.popupWherePieceDroppedMessage(pieceItem, Game.swordManager.getSwordsByPieceId(pieceItem.id), Game.swordManager.getFoundSwordIndexes());
}
